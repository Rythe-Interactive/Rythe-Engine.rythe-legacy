...
namespace legion::physics
{
    ...
    void PhysicsTestSystem::spawnRandomConvexHullOnCameraLocation(SpawnRandomHullOnCameraLoc& action)
    {
        ...
        //add a rigidbody
        dvr_internal_rigidbody& rb = ent.add_component<physics::dvr_internal_rigidbody>().get();
        ...
        auto& newPhysicsComponent = ent.add_component<physics::dvr_internal_physics_component>().get();
    ...
    void PhysicsTestSystem::addStaircase(math::vec3 position, float breadthMult, float widthMult )
    {
        ...
        auto& entPhysicsComponent = ent.add_component<physics::dvr_internal_physics_component>().get();
    ...
    void PhysicsTestSystem::createQuickhullTestObject(math::vec3 position, rendering::model_handle cubeH, rendering::material_handle TextureH, math::mat3 inertia )
    {
        ...
        auto& entPhysicsComp = ent.add_component<physics::dvr_internal_physics_component>().get();
        entPhysicsComp.constructConvexHullFromVertices(cubeH.get_mesh()->vertices);

        auto& rb = ent.add_component<dvr_internal_rigidbody>().get();
    ...
    void PhysicsTestSystem::PopulateFollowerList(ecs::entity physicsEnt, int index)
    {
        ...
        auto& physicsComp = physicsEnt.get_component<dvr_internal_physics_component>().get();
    ...
    void PhysicsTestSystem::drawPhysicsColliders()
    {
        ...
        ecs::filter< physics::dvr_internal_physics_component,transform> physicsQuery;
        ...
        for (auto entity : physicsQuery)
        {
            ...
            auto& physicsComponent = entity.get_component<physics::dvr_internal_physics_component>().get();
            ...
            if (entity.get_component<physics::dvr_internal_rigidbody>())
            {
                usedColor = rbColor;
                useDepth = true;
            }
    ...
    void PhysicsTestSystem::quickHullStep(QHULL& action)
    {
        if (!action.value)
        {
            ...
            for (ecs::entity ent : registeredColliderColorDraw)
            {
                ...
                auto& physicsComponent = ent.get_component<physics::dvr_internal_physics_component>().get();
    ...
    void PhysicsTestSystem::AddRigidbodyToQuickhulls(AddRigidbody& action)
    {
        if (!action.value)
        {
            ...
            {
                ent.add_component<dvr_internal_rigidbody>();
            }
        }
    }
        ...
        rendering::material_handle materials, physics::cube_collider_params cubeParams, bool useQuickhull, bool rigidbody , float mass , math::mat3 inverseInertia )
    {
        ...
        if (rigidbody)
        {
            auto& rbH = ent.add_component<physics::dvr_internal_rigidbody>().get();
            rbH.setMass(mass);
